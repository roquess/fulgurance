...
/// A Least Recently Used (LRU) cache implementation with integrated prefetch strategies
///
...
where
    ...
    V: Clone,
{
    /// Creates a new LRU cache with no prefetch (baseline)
    ///
    /// # Arguments
    ...
    /// Creates a new LRU cache with custom prefetch strategy
    ///
    /// # Arguments
    /// * `capacity` - Maximum number of items the cache can hold
...
mod tests {
    ...
    #[test]
    fn test_lru_with_prefetch_basic() {
        let mut cache = LruCache::with_prefetch_i32(3, PrefetchType::Sequential);
    ...
    #[test]
    fn test_lru_with_markov_prefetch() {
        let mut cache = LruCache::with_prefetch_i32(4, PrefetchType::Markov);
    ...
    #[test]
    fn test_lru_custom_prefetch_strategy() {
        let custom_strategy = Box::new(NoPrefetch);
        let mut cache = LruCache::with_custom_prefetch(3, custom_strategy);
    ...
    #[test]
    fn test_lru_prefetch_buffer_size() {
        let mut cache = LruCache::with_prefetch_i32(4, PrefetchType::Sequential);
        
        // Test setting prefetch buffer size
        cache.set_prefetch_buffer_size(2);
        
        cache.insert(1, "one".to_string());
        cache.get(&1); // Trigger prefetch
        
        // Buffer management should work without panicking
        assert!(cache.prefetch_buffer.len() <= 2);
    }

    #[test]
    fn test_lru_eviction_with_prefetch() {
        let mut cache = LruCache::with_prefetch_i32(2, PrefetchType::Sequential);
    ...
    #[test]
    fn test_lru_different_key_types() {
        let mut cache_i64 = LruCache::with_prefetch_i64(3, PrefetchType::Markov);
        let mut cache_usize = LruCache::with_prefetch_usize(3, PrefetchType::Sequential);
