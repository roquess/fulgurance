...
fn bench_random_basic_operations(c: &mut Criterion) {
    let mut group = c.benchmark_group("random_basic");
...
fn bench_random_access_patterns(c: &mut Criterion) {
    let mut group = c.benchmark_group("random_patterns");
    let cache_size = 200;
    let data_size = 1000;
...
criterion_group!(benches, bench_random_basic_operations, bench_random_access_patterns);
