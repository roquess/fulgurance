...
fn bench_fifo_basic_operations(c: &mut Criterion) {
    let mut group = c.benchmark_group("fifo_basic");
...
fn bench_fifo_access_patterns(c: &mut Criterion) {
    let mut group = c.benchmark_group("fifo_patterns");
    let cache_size = 200;
    let data_size = 1000;
...
criterion_group!(benches, bench_fifo_basic_operations, bench_fifo_access_patterns);
